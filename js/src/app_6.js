function App(props) {
  ...
  for (let i = 1; i <= currentEdition.attemptsLimit; i++) {
  ...
  React.useEffect(() => {
    ...
    if (getFromLocalStorage("lastPlayedIssueNumber") == getIssueNumber()) {
      var localAttempts = tryLoadingFromLocalStorage("attempts", attempts, {setter: setAttempts});
      var localFeedback = tryLoadingFromLocalStorage("feedback", feedback, {setter: setFeedback});
      tryLoadingFromLocalStorage("result", result, {setter: setResult});
      setCursor({
  ...
  function getIssueNumber() {
    const diff = Math.ceil((getKyivDateTimeIgnoringGMT(new Date())-getKyivDateTimeIgnoringGMT(currentEdition.firstDay))/(1000*60*60*24))
    return diff;
  }
        ...
        <div id="selector-editions">
          ...
          <ul>
            {editions.map((edition) => 
              <li><a href={"/" + edition.lettersLimit + ".html"} className={"edition" + (edition.lettersLimit == currentEdition.lettersLimit ? " selected" : "")}>{edition.lettersLimit}</a></li>
            )}
          </ul>
        </div>
